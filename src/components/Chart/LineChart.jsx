...
const generateOptions = data => {
  ...
  return {
    ...
    xAxis: {
      categories: data.timeline && Object.keys(data.timeline.cases),
      crosshair: true
    },
    colors: ['#F3585B'],
    yAxis: {
    ...
    series: [
      {
        name: 'Số ca nhiễm',
        data: data.timeline && Object.values(data.timeline.cases)
        // pointStart: 1,
      }
    ]
  };
...
function LineChart(props) {
  const { countryReport, onTimeChange } = props;
  ...
  useEffect(() => {
    setOptions(generateOptions(countryReport));
  }, [countryReport, reportType]);
